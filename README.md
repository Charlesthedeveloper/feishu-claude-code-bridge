...
| `/model [default\|<model-id>]` | Set or clear the current session's model override. Claude supports aliases such as `best`, `opus`, `opus[1m]`, `sonnet`, `sonnet[1m]`, `sonnet5`, `haiku`, `fable`, `fable5` plus full IDs; Codex supports full IDs plus `sol`, `terra`, and `luna` GPT-5.6 shortcuts |
...
- `/model best|opus|opus[1m]|sonnet|sonnet[1m]|sonnet5|haiku|fable|fable5|default|<model-id>`: in a Claude profile, changes only the current chat/topic's Claude Code `--model` override. The bridge preserves Claude Code's native aliases (`best`, `opus`, `sonnet`, `haiku`, `fable`) and maps common explicit forms such as `sonnet5` to `claude-sonnet-5`, `fable5` to `claude-fable-5`, `sonnet-1m` to `sonnet[1m]`, and `opus-1m` to `opus[1m]`. On the Anthropic API, Claude Code currently resolves `sonnet` to Sonnet 5 and `opus` to Opus 4.8; use a full model ID to pin a specific version. `default` removes the current session override so it falls back to `/config`.
- `/model default|<model-id>`: in a Codex profile, changes only the current chat/topic's Codex `--model` override. GPT-5.6 shortcuts are supported: `/model gpt-5.6` (official Sol alias), `/model sol`, `/model terra`, and `/model luna`, resolving to `gpt-5.6-sol`, `gpt-5.6-terra`, and `gpt-5.6-luna`. GPT-5.6 is currently a limited preview and requires entitlement for the Codex workspace; a paid consumer subscription alone does not grant access.
