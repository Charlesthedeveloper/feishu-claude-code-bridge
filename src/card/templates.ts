  ...
  return shell('💡 使用帮助', [
    divMd(
      [
        ...
        '- `/effort [level|auto|default]` — 当前 session 的 reasoning effort；Claude 支持 low/medium/high/xhigh/max 和 ultracode session-only 模式，Codex 支持 none/minimal/low/medium/high/xhigh/max（max 需模型支持）',
        '- `/model [default|<model-id>]` — 当前 session 的模型覆盖；Codex 可用 gpt-5.6/sol/terra/luna，Claude 可用 best/opus/sonnet/fable 等 alias',
