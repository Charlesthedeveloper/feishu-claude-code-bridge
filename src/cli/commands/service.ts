...
export async function runServiceRestart(opts: ServiceProfileOptions = {}): Promise<void> {
  ...
  // Reinstall before the bounce so launchd/system services pick up the
  // current PATH, proxy environment, and supervisor/classic run arguments.
  ...
  if (adapter.isRunning()) {
    ...
    const ok = await adapter.waitUntilStopped();
    if (!ok) {
      console.error('✗ 旧后台实例没有完全停止。请稍后重试,或:');
      console.error('  unregister  # 强制清除注册');
      console.error('  start       # 再次启动');
      process.exit(1);
    }
    await reportConnectAfter('restarted', waitProfile, adapter.start);
    return;
  }
