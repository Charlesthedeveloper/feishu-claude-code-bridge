...
import { normalizeAgentModelForAgent } from '../config/schema';
...
const CLAUDE_MODELS: ModelOption[] = [
  ...
  { value: 'claude-opus-5', label: 'Opus 5（推荐默认 / SOTA）' },
  { value: 'claude-fable-5', label: 'Fable 5（最高复杂度任务）' },
  { value: 'claude-sonnet-5', label: 'Sonnet 5（高效日常任务）' },
...
const CODEX_MODELS: ModelOption[] = [
  { value: DEFAULT_MODEL, label: '跟随默认（不指定）' },
  { value: 'gpt-5.6-sol', label: 'GPT-5.6 Sol（推荐）' },
...
export function normalizeModelSelection(
  ...
  const normalized = normalizeAgentModelForAgent(value as string, agentKind);
  return normalized && supportedModels(agentKind).some((m) => m.value === normalized)
    ? normalized
    : DEFAULT_MODEL;
}
 ...
 * the `--model` flag. Obvious cross-agent values are rejected, while unknown
 * values pass through so newly released and private model ids keep working.
 */
export function resolveModelArg(
  agentKind: AgentKind,
  value: string | undefined,
): string | undefined {
  if (isDefaultModel(value)) return undefined;
  const normalized = normalizeAgentModelForAgent(value as string, agentKind);
  if (!normalized || normalized === DEFAULT_MODEL) return undefined;
  const lower = normalized.toLowerCase();

  if (
    (agentKind === 'codex' &&
      (lower.startsWith('claude-') ||
        ['opus', 'sonnet', 'haiku', 'fable', 'best', 'opusplan'].includes(lower))) ||
    (agentKind === 'claude' && (lower.startsWith('gpt-') || /^o\d/.test(lower)))
  ) {
    return undefined;
  }
...
export function modelLabel(agentKind: AgentKind, value: string | undefined): string {
  const normalized = resolveModelArg(agentKind, value);
  if (!normalized) {
    return supportedModels(agentKind).find((m) => m.value === DEFAULT_MODEL)?.label ?? DEFAULT_MODEL;
  }
