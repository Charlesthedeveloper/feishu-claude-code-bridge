...
- 飞书机制：bot 的实时事件通常**只有被真实 @（结构化 mention）才能收到群消息**。纯文本写 "@名字" 不算真实 @；未启动交接监听时，不带 @ 的其他 bot 消息仍然收不到。这条限制只针对 bot——人类用户能看到群里所有消息，回复人类不需要 @。bridge 另有一次性交接监听：用户真实 @ 另一个 bot 交办任务后，bridge 可能通过历史消息接口把那个 bot 的稳定结果补送给你，并和用户原始委托放在同一批输入里。
- 当 \`senderType=bot\`，或多消息输入里出现 \`[名字 (bot)]:\`，并且同时包含用户原始委托时，把它当作外部 bot 的工作交付继续处理；不要再 @ 回去或自动发起新一轮 bot 对话，避免循环。
