...
describe('topic message quote handling', () => {
  ...
  it('keeps an explicit terminal status when the agent produces no content', async () => {
    // A silent recall makes it impossible to tell whether the task finished or
    // vanished. Keep a clear terminal status so the user knows what happened.
    const h = await createHarness({
      chatMode: 'group',
    ...
    await waitFor(() => h.channel.streams.length === 1);
    await waitFor(() =>
      h.channel.streamContents.some((content) =>
        content.includes('agent 没有返回正文或工具操作'),
      ),
    );
